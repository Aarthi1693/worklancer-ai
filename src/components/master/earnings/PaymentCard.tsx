"use client";

import {
  Building2,
  CalendarDays,
  CheckCircle2,
  Clock3,
  Eye,
  IndianRupee,
  Lock,
  Wallet,
} from "lucide-react";

export default function PaymentCard() {
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm transition-all duration-300 hover:shadow-lg">

      {/* Header */}

      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">

        <div>
          <div className="mb-3 flex items-center gap-3">
            <h2 className="text-xl font-bold text-slate-900">
              Healthcare Management System
            </h2>

            <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700">
              Payment Pending
            </span>
          </div>

          <p className="text-sm text-slate-500">
            Payment will be released after the provider reviews and approves
            your submitted work.
          </p>
        </div>

        <div className="rounded-2xl border border-green-200 bg-green-50 p-5">
          <p className="text-sm text-slate-600">
            Project Earnings
          </p>

          <h2 className="mt-2 flex items-center text-3xl font-bold text-green-600">
            <IndianRupee className="mr-1 h-6 w-6" />
            50,000
          </h2>
        </div>

      </div>

      {/* Details */}

      <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-4">

        <div className="flex items-center gap-3">

          <div className="rounded-xl bg-blue-100 p-3">
            <Building2 className="h-5 w-5 text-blue-600" />
          </div>

          <div>
            <p className="text-xs text-slate-500">
              Provider
            </p>

            <p className="font-semibold text-slate-900">
              ABC Technologies
            </p>
          </div>

        </div>

        <div className="flex items-center gap-3">

          <div className="rounded-xl bg-amber-100 p-3">
            <Lock className="h-5 w-5 text-amber-600" />
          </div>

          <div>
            <p className="text-xs text-slate-500">
              Escrow Status
            </p>

            <p className="font-semibold text-amber-600">
              Locked
            </p>
          </div>

        </div>

        <div className="flex items-center gap-3">

          <div className="rounded-xl bg-purple-100 p-3">
            <Clock3 className="h-5 w-5 text-purple-600" />
          </div>

          <div>
            <p className="text-xs text-slate-500">
              Payment Status
            </p>

            <p className="font-semibold text-purple-600">
              Awaiting Approval
            </p>
          </div>

        </div>

        <div className="flex items-center gap-3">

          <div className="rounded-xl bg-green-100 p-3">
            <CalendarDays className="h-5 w-5 text-green-600" />
          </div>

          <div>
            <p className="text-xs text-slate-500">
              Expected Release
            </p>

            <p className="font-semibold text-slate-900">
              28 Jul 2026
            </p>
          </div>

        </div>

      </div>

      {/* Progress */}

      <div className="mt-8">

        <div className="mb-2 flex justify-between text-sm">
          <span className="font-medium text-slate-700">
            Payment Progress
          </span>

          <span className="font-semibold text-blue-600">
            80%
          </span>
        </div>

        <div className="h-3 rounded-full bg-slate-200">
          <div className="h-3 w-4/5 rounded-full bg-blue-500"></div>
        </div>

      </div>

      {/* Timeline */}

      <div className="mt-8 rounded-2xl bg-slate-50 p-5">

        <h3 className="mb-4 font-semibold text-slate-900">
          Payment Timeline
        </h3>

        <div className="flex flex-wrap items-center gap-2 text-sm">

          <span className="rounded-full bg-green-100 px-3 py-1 text-green-700">
            Assigned
          </span>

          →

          <span className="rounded-full bg-green-100 px-3 py-1 text-green-700">
            Work Submitted
          </span>

          →

          <span className="rounded-full bg-blue-100 px-3 py-1 text-blue-700">
            Under Review
          </span>

          →

          <span className="rounded-full bg-slate-200 px-3 py-1 text-slate-600">
            Payment Release
          </span>

          →

          <span className="rounded-full bg-slate-200 px-3 py-1 text-slate-600">
            Withdraw
          </span>

        </div>

      </div>

      {/* Actions */}

      <div className="mt-8 flex flex-wrap justify-end gap-3">

        <button className="flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-5 py-3 text-slate-700 hover:bg-slate-100">
          <Eye size={18} />
          View Project
        </button>

        <button className="flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-3 text-white hover:bg-blue-700">
          <CheckCircle2 size={18} />
          Track Payment
        </button>

        <button
          disabled
          className="flex cursor-not-allowed items-center gap-2 rounded-xl bg-slate-200 px-5 py-3 text-slate-500"
        >
          <Wallet size={18} />
          Withdraw
        </button>

      </div>

    </div>
  );
}